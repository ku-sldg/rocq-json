...
SKIP_RATIONALES = {
    "parameterized-module-scope": "The declaration appears inside a parameterized module/functor body, so there is no closed global reference to derive until the functor is instantiated.",
    "module-type-scope": "The declaration appears in a module type/signature, which describes an interface component rather than a closed inductive declaration.",
}

SEMANTIC_NON_TARGET_CATEGORIES = {
...
def fmt_extraction_ms(value: float | None) -> str:
    if value is None:
        return "n/a"
    return f"{value * 1000:.3f}"


def fmt_percent_change(generated: float, handwritten: float) -> str:
    if handwritten == 0:
        return "n/a"
    return f"{((generated - handwritten) / handwritten) * 100:+.1f}" + r"\%"
...
def extraction_comparison_rows(rows: list[dict[str, str]]) -> list[str]:
    by_name: dict[str, list[float]] = {}
    for row in rows:
        by_name.setdefault(row["benchmark"], []).append(float(row["seconds"]))
    preferred_bases = [
    ...
    out: list[str] = []
    for base in bases:
        generated = by_name.get(f"{base}_generated")
        handwritten = by_name.get(f"{base}_handwritten")
        if not generated or not handwritten:
            continue
        generated_mean = statistics.mean(generated)
        handwritten_mean = statistics.mean(handwritten)
        out.append(
            " & ".join(
                [
                    latex_escape(base),
                    fmt_extraction_ms(generated_mean),
                    fmt_extraction_ms(handwritten_mean),
                    fmt_percent_change(generated_mean, handwritten_mean),
                ]
            )
            + r" \\"
        )
...
def main() -> int:
    ...
    skipped_categories = summary.get("skipped_categories", {})
    skipped_total = sum(int(count) for count in skipped_categories.values())
    summary["skipped_total"] = skipped_total
    failure_categories = summary.get("failure_categories", {})
    semantic_non_targets = sum(
        int(count)
        for category, count in failure_categories.items()
        if category in SEMANTIC_NON_TARGET_CATEGORIES
    )
    probe_successes = int(summary.get("probe_successes", 0))
    probe_failures = int(summary.get("probe_failures", 0))
    target_fragment = probe_successes + probe_failures - semantic_non_targets
    target_limitations = max(target_fragment - probe_successes, 0)
    ...
    lines = [
        ...
        command("JsonifiableSemanticNonTargets", semantic_non_targets),
        command("JsonifiableTargetFragment", target_fragment),
        command("JsonifiableDependentProofLimitations", target_limitations),
        command(
            "JsonifiableTargetCoverage",
            fmt_percent(probe_successes, target_fragment),
        ),
    ...
    lines.extend(
        [
            ...
            *extraction_comparison_rows(extraction_rows),
            "}",
            "",
        ]
    )
